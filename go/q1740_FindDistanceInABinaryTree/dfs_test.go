...
func Test_dfs_findDistance_1(t *testing.T) {
	assert := assert.New(t)
	...
	ans := findDistance(root, 5, 0)

	assert.Equal(3, ans, "There are 3 edges between 5 and 0: 5-3-1-0.")
}

func Test_dfs_findDistance_2(t *testing.T) {
	assert := assert.New(t)
	...
	ans := findDistance(root, 5, 7)

	assert.Equal(2, ans, "There are 2 edges between 5 and 7: 5-2-7.")
}

func Test_dfs_findDistance_3(t *testing.T) {
	assert := assert.New(t)
